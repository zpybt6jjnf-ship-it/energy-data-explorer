...
/**
 * Get member data points for a group (for background lines feature)
 */
...
/**
 * Calculate statistics for an aggregated group
 */
export function calculateGroupStats(
  memberData: StateDataPoint[]
): {
  saidiMin: number | null;
  saidiMax: number | null;
  saidiStdDev: number | null;
  vreMin: number;
  vreMax: number;
} {
  const saidiValues = memberData
    .map(d => d.saidi)
    .filter((v): v is number => v !== null);

  const vreValues = memberData.map(d => d.vrePenetration);

  const saidiMin = saidiValues.length > 0 ? Math.min(...saidiValues) : null;
  const saidiMax = saidiValues.length > 0 ? Math.max(...saidiValues) : null;

  // Calculate standard deviation for SAIDI
  let saidiStdDev: number | null = null;
  if (saidiValues.length > 1) {
    const mean = saidiValues.reduce((a, b) => a + b, 0) / saidiValues.length;
    const variance =
      saidiValues.reduce((sum, v) => sum + Math.pow(v - mean, 2), 0) /
      (saidiValues.length - 1);
    saidiStdDev = Math.sqrt(variance);
  }
  ...
  return {
    ...
    saidiMax,
    saidiStdDev,
    vreMin: Math.min(...vreValues),
    vreMax: Math.max(...vreValues)
  };
}
